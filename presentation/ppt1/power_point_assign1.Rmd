---
title: "Evaluating Countries based on Environmental Conditions"
output:
...
```{r LoadingPackages, echo=FALSE, message=FALSE, include=FALSE}
...
```{r ExploringData, include=FALSE, results='hide'}
country_env_df = read.csv('../dataset/World_countries_env_vars.csv')
str(country_env_df)
```

* Data: Measurements of environment conditions in Countries
* Rows: There are `r nrow(country_env_df)` observations, 1 for each country
* Columns: Total `r nrow(country_env_df)` variables. 
...
```{r headrows, results='asis', echo=FALSE}
head(country_env_df, n = 6)
```
