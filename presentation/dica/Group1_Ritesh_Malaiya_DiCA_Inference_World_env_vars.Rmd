...
country_env_mca <- tepDICA(DATA = as.matrix(country_env_df_for_mca), DESIGN = happiness_dummies, graphs = FALSE,symmetric = TRUE)
...
```{r}
corr_result = cor(t(happiness_dummies) %*% as.matrix(country_env_df_for_mca))
...
comp1 = data.frame(lx1 = country_env_mca$TExPosition.Data$lx[,1], 
                   ly1 = country_env_mca$TExPosition.Data$ly[,1])
mean_comp1 = comp1
...
country_map = country_factor_map$zeMap + country_factor_mean_map$zeMap_dots + country_factor_mean_map$zeMap_text + ggtitle('Component 1') #+ country_label4Map
